...
<body>
    ...
    <div class="page-title-area py-lg-6 py-5 bg-image-pattern">
        <div class="container">
            <div class="page-title-wrapper text-center">
                <h1 class="text-white mb-2">Kategori Artikel</h1>
    ...
    $(document).ready(function() {
        ...
        function loadMoreData() {
            $.ajax({
                url: "<?= base_url('blog/loadMoreKategori/') ?>" + offset,
                type: 'GET',
                beforeSend: function() {
                ...
                success: function(response) {
                    ...
                    if (kagetoriData.length > 0) {
                        for (var i = 0; i < kagetoriData.length; i++) {
                            ...
                            var tanggal = format_indo(kagetoriData[i].tanggal_dibuat);

                            $('#kategori-container').append(
                                '<div class="blog-style mt-4"><div class="blog-image"><img src="<?= base_url("upload/artikel/") ?>' +
                                kategoriData[i].gambar +
                                '" alt="kategori"></div><div class="blog-content"><span class="blog-meta"><i class="fas fa-tag me-2"></i><a href="<?= base_url("blog/kategori/") ?> ' +
                                kategoriData[i].nama_kategori + '">' + kategoriData[i]
                                .nama_kategori +
                                '</a></span><h4><a href="<?= base_url('blog/detail/') ?> ' +
                                kategoriData[i].slug + '">' + kagetoriData[i].judul +
                                '</a></h4><small class="text-muted"><i class="fa fa-clock"></i>' +
                                tanggal + '</small><p class="mt-2">' + deskripsi +
                                '</p></div></div>'
                            );
                        }
