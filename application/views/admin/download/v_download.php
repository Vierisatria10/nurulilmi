<!-- Content Header (Page header) -->
<style>
.btn-green {
    background-color: #0F7571;
    color: #fff;
}

.btn-green:hover {
    background-color: #0F7571;
    color: #fff;
}
</style>
<div class="content-header">
    <div class="container-fluid">
        <div class="row mb-2">
            <div class="col-sm-6">
                <h1 class="m-0 text-dark"><?= $judul; ?></h1>
            </div><!-- /.col -->
            <div class="col-sm-6">
                <ol class="breadcrumb float-sm-right">
                    <li class="breadcrumb-item"><a href="#">Home</a></li>
                    <li class="breadcrumb-item active"><?= $judul; ?></li>
                </ol>
            </div><!-- /.col -->
        </div><!-- /.row -->
    </div><!-- /.container-fluid -->
</div>
<!-- /.content-header -->

<!-- Main content -->
<section class="content">
    <div class="container-fluid">
        <!-- Main row -->
        <div class="col-12">
            <!-- /.card -->
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">Data File Download</h3>
                    <?= $this->session->flashdata('message') ?>
                    <div class="d-flex justify-content-end">
                        <a href="#" data-target="#add_file" data-toggle="modal" class="btn btn-green btn-sm"><i
                                class="fas fa-plus"></i> Tambah File</a>
                    </div>
                </div>
                <!-- /.card-header -->
                <div class="card-body">
                    <table id="imam" class="table table-bordered table-striped">
                        <thead>
                            <tr>
                                <th>No</th>
                                <th class="text-center">Nama File</th>
                                <th class="text-center">Tanggal Dibuat</th>
                                <th class="text-center">File</th>
                                <th>Aksi</th>
                            </tr>
                        </thead>
                        <tbody>
                            <?php $no = 1; ?>
                            <?php foreach ($data_download as $download) : ?>
                            <tr>
                                <td><?= $no++; ?></td>
                                <td class="text-center"><?= $download->nama_file ?></td>
                                <td class="text-center"><?= $download->tgl_dibuat ?></td>
                                <td class="text-center">
                                    <?php if(!empty($download->file)) : ?>
                                    <a href="<?= base_url('upload/download/' . $download->file) ?>" target="_blank">
                                        <i class="fa fa-file-download fa-2x"></i>
                                    </a>
                                    <?php else: ?>
                                    <span class="text-danger">Tidak ada File</span>
                                    <?php endif; ?>
                                </td>
                                <td>
                                    <a href="" data-toggle="modal" data-target="#edit_file<?= $download->id_download ?>"
                                        class="m-1 btn btn-info btn-sm"><i class="fas fa-fw fa-edit"></i>
                                        Ubah</a>
                                    <a href="" data-toggle="modal"
                                        data-target="#hapus_download<?= $download->id_download ?>"
                                        class="btn-delete m-1 btn btn-danger btn-sm"
                                        data-text="<?= $download->nama_file; ?>"><i class="fas fa-fw fa-trash"></i>
                                        Hapus</a>
                                </td>
                            </tr>
                            <?php endforeach ?>
                        </tbody>
                    </table>
                </div>
                <!-- /.card-body -->
            </div>
            <!-- /.card -->
        </div>
        <!-- /.row (main row) -->
    </div><!-- /.container-fluid -->
</section>

<!-- Modal add -->
<div class="modal fade" id="add_file" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
    aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">Form File</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form action="<?= base_url('admin/download/simpan_download') ?>" method="post"
                    enctype="multipart/form-data">
                    <div class="row">
                        <div class="col-md-12">
                            <div class="form-group">
                                <label for="">Nama File</label>
                                <input type="text" class="form-control" name="nama_file">
                            </div>
                        </div>
                        <div class="col-md-12">
                            <div class="form-group">
                                <label for="">File</label>
                                <input type="file" class="form-control" name="file">
                                <small class="text-danger">*Upload file wajib pdf|xlsx|docx|xls</small>
                            </div>
                        </div>
                    </div>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                <button type="submit" class="btn btn-green">Simpan</button>
            </div>
            </form>
        </div>
    </div>
</div>

<!-- Modal edit -->
<?php foreach($data_download as $download) : ?>
<div class="modal fade" id="edit_file<?= $download->id_download ?>" tabindex="-1" role="dialog"
    aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">Form File</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form action="<?= base_url('admin/download/update_download/'.$download->id_download) ?>" method="post"
                    enctype="multipart/form-data">
                    <div class="row">
                        <div class="col-md-12">
                            <div class="form-group">
                                <label for="">Nama File</label>
                                <input type="text" class="form-control" value="<?= $download->nama_file ?>"
                                    name="nama_file">
                            </div>
                        </div>
                        <div class="col-md-12">
                            <div class="form-group">
                                <label for="">File Sebelumnya</label>
                                <br>
                                <?php if(!empty($download->file)) : ?>
                                <a href="<?= base_url('upload/download/'. $download->file) ?>" target="_blank">
                                    <i class="fa fa-file-download fa-2x"></i>
                                </a>
                                <?php else: ?>
                                <img src="<?= base_url('upload/default.png') ?>" alt="">
                                <?php endif; ?>
                            </div>
                        </div>
                        <div class="col-md-12">
                            <div class="form-group">
                                <label for="">File</label>
                                <input type="hidden" class="form-control" name="file_old"
                                    value="<?= $download->file ?>">
                                <input type="file" class="form-control" name="file">
                                <small class="text-danger">*Upload file wajib pdf|xlsx|docx|xls</small>
                            </div>
                        </div>
                    </div>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                <button type="submit" class="btn btn-green">Simpan</button>
            </div>
            </form>
        </div>
    </div>
</div>
<?php endforeach; ?>

<!-- Modal delete -->
<?php foreach($data_download as $download) : ?>
<div class="modal fade" id="hapus_download<?= $download->id_download ?>" tabindex="-1" role="dialog"
    aria-labelledby="hapus_download<?= $download->id_download ?>" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="hapus_download<?= $download->id_download ?>">Apakah kamu ingin menghapus
                    data ini?
                </h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form action="<?= base_url('admin/download/delete/'.$download->id_download) ?>" method="POST"
                    enctype="multipart/form-data">
                    <input type="hidden" id="id_download" name="id_download" value="<?= $download->id_download ?>">
                    <p class="text-danger">Menghapus Data File Download yang bernama :
                        <b><?= $download->nama_file; ?></b>
                    </p>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                <button type="submit" class="btn btn-danger"><i class="fas fa-trash"></i> Hapus</button>
            </div>
            </form>
        </div>
    </div>
</div>
<?php endforeach; ?>